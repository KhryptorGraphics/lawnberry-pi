...
async def test_settings_maps_provider_osm_fallback():
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        payload = {"provider": "osm", "zoom_level": 18}
...
async def test_settings_camera_resolution_validation():
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        payload_valid = {"width": 1920, "height": 1080, "framerate": 30}
        response_valid = await client.put(
...
async def test_settings_system_sim_mode_toggle():
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        # Disable SIM_MODE
        payload_disable = {"sim_mode": False}
...
async def test_settings_ai_model_selection_validation():
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        payload_valid = {"model": "yolov8n", "confidence_threshold": 0.5}
        response_valid = await client.put(
            "/api/v2/settings/ai",
            json=payload_valid,
            headers=headers,
        ...
        payload_invalid = {"model": "nonexistent-model", "confidence_threshold": 0.5}
        response_invalid = await client.put(
            "/api/v2/settings/ai",
            json=payload_invalid,
            headers=headers,
