...
async def test_telemetry_latency_budgets_pi5():
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        assert p95_latency <= 250, f"P95 latency {p95_latency}ms exceeds 250ms budget"

        p50_latency = data["latency_ms_p50"]
        assert p50_latency <= 200, f"P50 latency {p50_latency}ms exceeds 200ms"
...
async def test_telemetry_latency_budgets_pi4():
    ...
    try:
        ...
        async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
            ...
            p95_latency = data["latency_ms_p95"]
            assert p95_latency <= 350, f"P95 latency {p95_latency}ms exceeds 350ms budget for Pi 4B"
...
async def test_rtk_dropout_recovery_with_fallback():
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        has_gps_component = any(item.get("component_id") == "gps" for item in data["items"])
...
async def test_imu_orientation_metadata():
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        data = response.json()
        imu_items = [item for item in data["items"] if item.get("component_id") == "imu"]
        ...
        for imu_item in imu_items:
            ...
            assert any(
                key in metadata for key in ["quaternion", "roll", "pitch", "yaw", "orientation"]
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        if response.status_code == 200:
            ...
            assert (
                ...
                or "application/csv" in response.headers.get("content-type", "").lower()
            )
...
async def test_telemetry_documentation_link_on_degradation():
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        degraded_items = [
            item for item in data["items"] if item.get("status") in ["warning", "fault"]
        ]
