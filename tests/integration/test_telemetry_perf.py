...
class TestTelemetryPerformance:
    ...
    async def test_telemetry_loop_performance(self, mock_websocket_hub):
        ...
        async def measure_telemetry_loop():
            ...
            for _ in range(20):  # Run for 20 iterations
                ...
                # Count messages sent
                message_count = sum(
                    ws.send_text.call_count for ws in mock_websocket_hub.clients.values()
                )
        ...
        loop_summary = f"Telemetry loop - Avg: {avg_loop_time:.2f}ms, Max: {max_loop_time:.2f}ms"
        print(loop_summary)
    ...
    async def test_client_connection_scalability(self, mock_websocket_hub):
        ...
        connection_summary = (
            f"Connection - Avg: {avg_connection_time:.2f}ms, Max: {max_connection_time:.2f}ms"
        )
    ...
    async def test_disconnection_cleanup_performance(self, mock_websocket_hub):
        ...
        cleanup_summary = (
            f"Cleanup performance - Avg: {avg_cleanup_time:.2f}ms, Max: {max_cleanup_time:.2f}ms"
        )
...
class TestTelemetryLatency:
    ...
    async def test_end_to_end_latency(self):
        ...
        if latencies:
            ...
            latency_summary = (
                f"End-to-end latency - Avg: {avg_latency:.2f}ms, Max: {max_latency:.2f}ms"
            )
...
class TestTelemetryThroughput:
    ...
    async def test_maximum_throughput(self):
        ...
        throughput_summary = (
            f"Maximum throughput: {throughput:.0f} messages/second with {client_count} clients"
        )
