...
class L298NController:
    ...
    async def send_drive_command(self, command: DriveCommand) -> bool:
        ...
        try:
            ...
            logger.debug(
                f"L298N: L={left_duty}%({'FWD' if left_forward else 'REV'}), "
...
class MotorService:
    ...
    async def send_drive_command(
        ...
        if success:
            ...
            self.motor_control.right_motor_status = (
                MotorStatus.RUNNING
                if abs(command.right_motor_speed) > 0.01
    ...
    async def send_blade_command(
        ...
        if command.active:
            if not self.safety_system.check_safety_conditions(
                self.motor_control, sensor_data or {}
            ):
    ...
    async def emergency_stop(self) -> bool:
        ...
        stop_command = DriveCommand(
            left_motor_speed=0.0, right_motor_speed=0.0, control_mode=ControlMode.EMERGENCY_STOP
        )
