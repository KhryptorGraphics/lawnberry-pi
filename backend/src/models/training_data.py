...
class ObjectAnnotation(BaseModel):
    ...
    bounding_box: BoundingBoxAnnotation | None = None
    polygon_points: list[list[float]] | None = None  # [[x1,y1], [x2,y2], ...]
    center_point: list[float] | None = None  # [x, y]

    # Metadata
    annotator: str = "system"  # "system", "user", "auto"
    annotation_time: datetime = Field(default_factory=lambda: datetime.now(UTC))
...
class TrainingImage(BaseModel):
    ...
    gps_location: dict[str, float] | None = None  # {"lat": x, "lon": y}
    robot_state: dict[str, Any] | None = None  # Robot state when captured
    scene_context: str | None = None  # Description of scene
...
class DatasetExport(BaseModel):
    ...
    class_filter: list[str] | None = None  # Export only specific classes
...
class TrainingData(BaseModel):
    ...
    max_images: int | None = None  # Maximum images to collect
    ...
    def update_statistics(self):
        ...
        if total_annotations > 0:
            stats.class_distribution = {
                cls: (count / total_annotations) * 100 for cls, count in class_counts.items()
            }
    ...
    def create_export(self, format: DatasetFormat, **kwargs) -> DatasetExport:
        ...
        export = DatasetExport(dataset_id=self.dataset_id, format=format, **kwargs)
