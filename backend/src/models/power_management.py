...
class PowerManagement(BaseModel):
    ...
    def update_power_mode(self) -> PowerMode:
        ...
        if battery_pct <= self.critical_battery_threshold:
            ...
            self.sun_seeking_enabled
            and battery_pct <= self.charging_start_threshold
            and self.solar_status.power < 5.0
        ):  # Low solar input
    ...
    def calculate_runtime_estimate(self) -> float | None:
        ...
        remaining_wh = self.battery_status.remaining_ah * self.battery_status.voltage
    ...
    def is_charging_recommended(self) -> bool:
        ...
        return (
            self.battery_status.percentage <= self.charging_start_threshold
            or self.power_mode in [PowerMode.LOW_BATTERY, PowerMode.CRITICAL_BATTERY]
        )
