...
class CameraStream(BaseModel):
    ...
    def update_statistics(self, frame_duration_ms: float, processing_time_ms: float | None = None):
        ...
        if stats.frames_captured == 1:
            ...
            stats.average_frame_time_ms = (
                stats.average_frame_time_ms * (n - 1) + frame_duration_ms
            ) / n
        ...
        if processing_time_ms is not None:
            if stats.frames_captured == 1:
                ...
                stats.average_processing_time_ms = (
                    stats.average_processing_time_ms * (n - 1) + processing_time_ms
                ) / n
    ...
    def is_healthy(self) -> bool:
        ...
        # Check error rate
        if (
            self.statistics.frames_captured > 100
            and self.statistics.frames_dropped / self.statistics.frames_captured > 0.1
        ):
