...
class TelemetryHub(BaseModel):
    ...
    def subscribe_client(self, client_id: str, topic: TelemetryTopic, **kwargs) -> bool:
        ...
        existing = next(
            (
                sub
                for sub in self.client_subscriptions
        ...
        topic_subscribers = sum(1 for sub in self.client_subscriptions if sub.topic == topic)
        if topic_subscribers >= self.streams[topic].max_subscribers:
            return False

        # Add subscription
        subscription = ClientSubscription(client_id=client_id, topic=topic, **kwargs)
...
class TelemetryExchange(BaseModel):
    ...
    def create_default_exchange(cls) -> "TelemetryExchange":
        ...
        for topic, cadence, critical in default_streams:
            config = StreamConfiguration(topic=topic, cadence_hz=cadence, critical_stream=critical)
